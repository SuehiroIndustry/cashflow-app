...
function pickNumber(row: Record<string, unknown>, keys: string[]): number {
  for (const k of keys) {
    if (k in row) return toNumber(row[k]);
  }
  return 0;
}
...
export async function GET(req: Request) {
  ...
  // plain が 0 で 30d のみあるパターンは plain を 30d に合わせる（UIがどっち見ても破綻しない）
  ...
  const projected_balance =
    agg.projected_balance || agg.projected_balance_30d || agg.current_balance + planned_income - planned_expense;
