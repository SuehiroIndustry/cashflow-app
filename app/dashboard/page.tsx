...
import { getDashboardOverview, DashboardOverviewRow } from "@/lib/dashboard/getDashboardOverview";

type Account = { id: number; name: string }; // 既存に合わせてOK

export default function DashboardPage() {
  // 例：口座リスト（既に取得してるならそれを使う）
  ...
  return (
    <div className="p-6 space-y-6">
      ...
      <div className="flex gap-2 flex-wrap">
        <button
          className={`px-3 py-1 rounded-full border ${selected.mode === "all" ? "opacity-100" : "opacity-60"}`}
        ...
        {accounts.map((a) => (
          <button
            ...
            className={`px-3 py-1 rounded-full border ${
              selected.mode === "account" && selected.cashAccountId === a.id ? "opacity-100" : "opacity-60"
            }`}
