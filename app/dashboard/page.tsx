...
export default async function DashboardPage({
  ...
  async function insertTransaction(formData: FormData) {
    ...
    // DashboardとSummary(API)は force-dynamic なので、これで十分
  ...
  summaryUrl.searchParams.set("account", isAllView ? "all" : (filterAccountId ?? "all"));
        ...
        <form action={insertTransaction} className="grid grid-cols-1 gap-4 md:grid-cols-2">
          ...
          <div className="md:col-span-2">
            ...
            <div className="mt-2 text-xs text-neutral-500">
              ● Summaryは /api/summary（VIEW）を参照。transactions を手計算しない。
            </div>
          </div>
        </form>
      </div>
